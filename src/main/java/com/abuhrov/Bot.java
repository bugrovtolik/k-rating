...
public class Bot extends TelegramLongPollingBot {
	...
	private ReplyKeyboard getDefaultReply() {
		return ReplyKeyboardBuilder.createReply().row().addText(NEW_RESULT_UKR).row().addText(SHOW_RATING_UKR).row().addText(ADD_PLAYER_UKR).row().addText(ABOUT_BOT_UKR).row().addText(CLEAR_RESULTS_UKR).build();
	}
