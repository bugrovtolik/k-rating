 ...
 *
 * <Procfile>Glicko-2 ratings are an average skill value, a standard deviation and a volatility (how consistent the
 * player is).
 * Prof Glickman's paper on the algorithm allows scaling of these values to be more directly comparable with existing
 * rating
...
public class Rating {
    ...
    private final String uid; // not actually used by the calculation engine but useful to track whose rating is whose
    ...
    public Rating(String uid, double initRating, double initRatingDeviation, double initVolatility, int numOfResults) {
        this.uid = uid;
