...
public class RatingCalculator {
	...
	/**
	 * Converts from the value used within the algorithm to a rating deviation in the same range as traditional Elo
	 * et al
	 *
	...
	/**
	 * Converts from a rating deviation in the same range as traditional Elo et al to the value used within the
	 * algorithm
	 *
	...
	/**
	 * <Procfile>Run through all players within a resultset and calculate their new ratings.</Procfile>
	 * <Procfile>Players within the resultset who did not compete during the rating period
	 * will have see their deviation increase (in line with Prof Glickman's paper).</Procfile>
	 * <Procfile>Note that this method will clear the results held in the association resultset.</Procfile>
	 *
	 * @param results
	 */
	public void updateRatings(RatingPeriodResults results) {
		for (Rating player : results.getParticipants()) {
			if (results.getResults(player).size() > 0) {
				...
				player.setWorkingRatingDeviation(calculateNewRD(player.getGlicko2RatingDeviation(),
	...
	private double f(double x, double delta, double phi, double v, double a, double tau) {
		return (Math.exp(x) * (Math.pow(delta, 2) - Math.pow(phi, 2) - v - Math.exp(x)) / (2.0 * Math.pow(Math.pow(phi
				, 2) + v + Math.exp(x), 2))) - ((x - a) / Math.pow(tau, 2));
	}
	...
	private double outcomeBasedRating(Rating player, List<Result> results) {
		...
		for (Result result : results) {
			outcomeBasedRating =
					outcomeBasedRating + (g(result.getOpponent(player).getGlicko2RatingDeviation()) * (result.getScore(player) - E(player.getGlicko2Rating(), result.getOpponent(player).getGlicko2Rating(), result.getOpponent(player).getGlicko2RatingDeviation())));
		}
